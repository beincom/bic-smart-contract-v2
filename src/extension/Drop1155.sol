...
abstract contract Drop1155 is IDrop1155, ReentrancyGuard {
    ...
    function setClaimConditions(
        ...
        for (uint256 i = 0; i < conditionsLength; ) {
            ClaimCondition calldata condition = _conditions[i];
            require(i == 0 || lastConditionStartTimestamp < condition.startTimestamp, "ST");

            uint256 supplyClaimedAlready = conditionList.conditions[newStartIndex + i].supplyClaimed;
            if (supplyClaimedAlready > condition.maxClaimableSupply) {
                revert DropExceedMaxSupply();
            }

            conditionList.conditions[newStartIndex + i] = condition;
            conditionList.conditions[newStartIndex + i].supplyClaimed = supplyClaimedAlready;

            lastConditionStartTimestamp = condition.startTimestamp;
